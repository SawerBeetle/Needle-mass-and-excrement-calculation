...
Для регрессионного анализа: `nls2`, `minpack.lm`, `lme4`. Для создания графиков: `ggplot2`, `cowplot`, `ggridges`. Для манипуляции данными: `readxl`, `tidyr`. 
...
```{r abies_read_data, warning=FALSE, message=FALSE}
# load raw data for model validation (Usoltsev, 2010) from .csv file
raw_as <- read_excel('Масса хвои пихты.xlsx')[, 2:6]
```
...
plot_rvol_vs_resid <- predicted_plot_2(
  raw_as$rel_volume, 
  res_as$nlm_resid, 
  'Зависимость остатков от отн. объёма стволов', 
...
Данные по ТХ и массе хвои по [@usolcev2010] для южной тайги Алтае-Саянской горной провинции загрузим из локального файла `.csv`.

```{r psib_read_data, warning=FALSE, message=FALSE}
# load raw data for model validation (Usoltsev, 2010) from .csv file
raw_ps <- read_excel('Масса хвои кедра.xlsx')[, 2:6]
raw_ps <- raw_ps[-4, ]
```

Не буду я нелинейную модель рассчитывать, лень. Пихта показала (ниже), что головной боли много, а толку мало. Здесь ограничимся загрузкой данных.
...
Результаты безрадостные: 

- смещение результатов (завышение низких, занижение высоких); 
- хвост влево.

Правда, гетероскедастичность не диагностируется, по крайней мере, на глаз. 

```{r psib_df_for_results}
# Before needle mass calculation, let's create the df for results. 
# In this frame we will put results of linear modelling and or 
# calculation through conversion factors (by Zamolodchikov et al.). 
res_ps <- data.frame(
  lm = rep(NA, nrow(raw_ps)), 
  lm_resid = rep(NA, nrow(raw_ps)), 
  cf = rep(NA, nrow(raw_ps)), 
  cf_resid = rep(NA, nrow(raw_ps))
)
```

```{r psib_linear_model, include=TRUE, fig.height=7.5, fig.width=7.5}
# create linear model (take into account the interaction between predictors)
model_needles_lm_ps <- lm(needle_mass ~ age * volume, raw_ps) 

# Put into results df 'res_as' the predicted needle masses and
res_ps$lm <- predict(model_needles_lm_ps)
# residuals. 
res_ps$lm_resid <- res_ps$lm - raw_ps$needle_mass

# coefficients of linear model and their characteristics
knitr::kable(
  data.frame(summary(model_needles_lm_ps)$coefficients), 
  caption = ('Коэффициенты линейной модели')
  )
...
plot_real_vs_model <- predicted_plot_2(
  raw_ps$needle_mass, 
  res_ps$lm, 
  'Реальные и модельные массы хвои кедра', 
  'Реальные данные', 
  'Данные модели'
  )
# real needle masses vs residuals, 
...
# density of residuals.
plot_dens <- ggplot(res_ps, aes(lm_resid)) + 
  geom_density() + 
  geom_rug() + 
  labs(title = 'Распределение остатков') + 
  xlab(element_blank()) + 
...
К старым претензиям (гетероскедастичность остатков, смещённость оценок) добавляется ещё и далёкое от нормального распределение остатков (длинный хвост вправо).
...
Пробы ставить негде. Завышение не то, что в разы – на порядки. 

```{r psib_conversion_factor, echo=FALSE, include=TRUE, warning = FALSE, message = FALSE, fig.height=7.5, fig.width=7.5}
# In the loop calculate the needle masses by age-related conversion factors: 
for(i in 1:nrow(raw_ps)) {
  # for young stands, 
  if(raw_ps$age[i] <= 80) res_ps$cf[i] <- .068 * raw_ps$volume[i]
  # for middle-aged stands, 
  if(raw_ps$age[i]>80 & raw_ps$age[i]<=120) res_ps$cf[i] <- .027 * raw_ps$volume[i]
  # for premature stands and
  if(raw_ps$age[i]>120 & raw_ps$age[i]<=160) res_ps$cf[i] <- .027 * raw_ps$volume[i]
  # for mature stands.
  if(raw_ps$age[i] > 160) res_ps$cf[i] <- .045 * raw_ps$volume[i]
}
# remove counter
rm(i)

# calculate residuals
res_ps$cf_resid <- res_ps$cf - raw_ps$needle_mass
...
# density of residuals.
plot_dens <- ggplot(res_ps, aes(cf_resid)) + 
  geom_density() + 
  geom_rug() + 
  labs(title = 'Распределение остатков') + 
  xlab(element_blank()) + 
...
Однозначно линейная модель. 

```{r psib_resid_comparison, include=TRUE, echo=FALSE, warning=FALSE, fig.height=3.7, fig.width=5}
# create the table with basic descriptive stats for LM and CF
knitr::kable(
  data.frame(
    LM = round(as.vector(summary(res_ps$lm_resid)), digits=2)[1:6], 
    CF = round(as.vector(summary(res_ps$cf_resid)), digits=2)[1:6],
    row.names = names(summary(res_ps$lm_resid))[1:6]
  ), 
  caption=('Описательные статистики для остатков')
)

# Add ggridges plot with the same data. First, create df, 
forplot <- res_ps[, c(2, 4)]
# give the appropriate names of columns and 
colnames(forplot) <- c('LM', 'CF') 
# make the long form of data frame. 
forplot <- pivot_longer(forplot, cols = 1:2)
# Now create a ggridge plot. First, the backbone.
ggplot(forplot, aes(x=value, y=name, fill=factor(stat(quantile)))) + 
  # Second, the geom with coloured tales:
  stat_density_ridges(
...
1. Рассчитываем массу хвои на гектар древостоя. Или с помощью линейной формулы, или используя переходные коэффициенты – сама решай. 
2. Массу хвои умножаем на степень дефолиации, чтобы узнать, сколько хвои было изъято. 
3. Эту массу умножаем на зависящий от породы коэффициент, чтобы получить массу зоогенного опада. 
4. Массу зоогенного опада умножаем на переводной коэффициент 0,337, чтобы получить массу эмитированного из него углерода. 
5. Разбиваем эмиссию по годам: в первый год (год дефолиации) она составляет 79,1% от общей, во второй 16,5%, в третий 4,4%. 

## Пихта

Рассчитываем массу хвои или с помощью линейной регрессии: 

$$m_f = 1.046 - 0.00167A + 0.0478V - 0.000214AV,$$

или используя переходные коэффициенты: 
$$A \le 40 ⇒ m_f = 0.138V,$$
$$A > 40 \cap A \le 60 ⇒ m_f = 0.047V,$$
$$A > 60 \cap A \le 80 ⇒ m_f = 0.037V,$$
$$A > 80 ⇒ m_f = 0.035V.$$

Теперь умножаем $m_f$ на степень дефолиации (в долях единицы) и два объединённых переводных коэффициента. Так мы получим массу эмитированного из хвои углерода из $m_f$, минуя промежуточные этапы расчёта массы изъятой хвои и зоогенного опада: 
...
$$m_f = 1.268 + 0.0518A - 0.0034V - 0.000052AV,$$

или используя переходные коэффициенты: 
$$A \le 80 ⇒ m_f = 0.068V,$$
$$A > 80 \cap A \le 120 ⇒ m_f = 0.027,$$
$$A > 120 \cap A \le 160 ⇒ m_f = 0.027V,$$
$$A > 160 ⇒ m_f = 0.045V.$$
...
## Распределение эмиссии углерода из хвои по сезонам

Теперь раскидаем $E_C$ по сезонам ($E_{C1}$ – сезон дефолиации и т.д.): 
